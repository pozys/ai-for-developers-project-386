...
describe('CalendarGrid', () => {
  ...
  it('отключает выходные и даты вне 14-дневного окна', () => {
    ...
    expect(screen.getByRole('button', { name: /19 апреля 2026/i })).toBeDisabled();
    expect(screen.getByRole('button', { name: /28 апреля 2026/i })).toBeDisabled();
  });
  ...
  it('позволяет перейти к следующему месяцу, если в нем есть доступные даты', async () => {
    ...
    expect(screen.getByRole('heading', { name: /май 2026/i })).toBeInTheDocument();
  });
});
