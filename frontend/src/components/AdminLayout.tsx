...
export default function AdminLayout() {
  return (
    <div className="space-y-6">
      <nav aria-label="Навигация по админке" className="flex flex-wrap gap-2 border-b border-border/60 pb-4">
        <NavLink to="/admin/event-types" className={({ isActive }) => getAdminNavClass(isActive)}>
          Типы событий
        </NavLink>
        <NavLink to="/admin/bookings" className={({ isActive }) => getAdminNavClass(isActive)}>
          Бронирования
        </NavLink>
      </nav>
      <Outlet />
