...
export function isDateKeyWithinBookingWindow(dateKey: string, now = new Date()) {
...
export function addDateKey(dateKey: string, days: number) {
  return formatUtcDateKey(addDaysToUtcDate(getUtcDateFromDateKey(dateKey), days))
}
...
export function getCalendarDays(monthStart: Date, now = new Date()): CalendarDay[] {
  ...
  return Array.from({ length: 42 }, (_, index) => {
    ...
    return {
      ...
      isSelectable: isDateKeyWithinBookingWindow(dateKey, now) && isWeekdayDateKey(dateKey),
    }
  })
}
